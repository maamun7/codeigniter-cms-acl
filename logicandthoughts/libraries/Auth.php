...
class Auth {
	...
	public function admin_login($username,$password)
	{
		...
		$result = $CI->user->check_valid_user($username,$password);				
        if ($result)
		{
			$key = md5(time());
	...
	function check_system_auth($url='')
	{   
	...
	public function is_system_user()
	{
		$CI =& get_instance();
        $CI->load->model('admin/user');
		$type = $CI->user->check_system_user($this->get_user_id());
	...
	public function is_admin()
	{
		$CI =& get_instance();
        $CI->load->model('admin/user');
		$type = $CI->user->check_by_user_id($this->get_user_id());
    ...
    function check_permission($slug, $redirect=true, $user_id = 0)
    {
        ...
        if ( $role_id === '1')
            return true;
        $res = $CI->user->get_by_role_id_and_slug($role_id,$slug);
        if( count($res) > 0){
            return true;
        }else{
            if( $redirect )
    ...
    private function has_role($user_id){
        $CI =& get_instance();
        $CI->load->model('admin/user');
        $role = $CI->user->get_by_user_id($user_id);

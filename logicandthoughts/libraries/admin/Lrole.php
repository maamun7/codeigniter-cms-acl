...
class Lrole {
	public function generate_role_list()
	{
		...
		$role_list = $CI->role->get_role_list();		
		$i=0;
		if(!empty($role_list)){		
		...
		$list_view = $CI->parser->parse('admin_view/roles/index',$data,true);
		return $list_view;
	}
	...
	public function new_role()
	{
		...
		$roleForm = $CI->parser->parse('admin_view/roles/add',$data,true);
		return $roleForm;
	}
	...
	public function role_edit_form($role_id)
	{
		$CI =& get_instance();
		$CI->load->model('admin/role');
		$role = $CI->role->get_role_edit_data($role_id);
		if(!empty($role)){
			$data['role_id'] = $role[0]['role_id'];
			$data['role'] = $role[0]['role'];
		}
		$data['title']='Edit Role';
		$roleEditForm = $CI->parser->parse('admin_view/roles/edit',$data,true);
		return $roleEditForm;
	} 
	...
	private function all_permission()
    {
        $CI =& get_instance();
        $CI->load->model('admin/role');
        $group = $CI->role->get_all_group();
        $i = -1;
            $custom = array();
            foreach($group as $k => $v){
                $permision = $CI->role->get_by_group_id($v['group_id']);
      ...
      $permissions = $CI->parser->parse('admin_view/roles/all_permissions', array('groups'=>$custom), TRUE);
      return $permissions;
    }
	...
	public function permission_form($role_id)
	{
        ...
        $role = $CI->role->get_by_role_id($role_id);
        $cdata = array('groups'=>$data,'role_name'=>$role[0]['role'],'role_id'=>$role_id);
        $content = $CI->parser->parse('admin_view/roles/permission', $cdata, TRUE);
        return $content;
	}
	...
	public function get_permissions($role_id)
	{
        $CI =& get_instance();
		$CI->load->model('admin/role');
        $res = $CI->role->get_permission_by_role_id($role_id);

...
class Luser {
	...
	public function generate_list($limit,$page,$links)
	{
		...
		$list = $CI->user->get_user_list($limit,$page);		
		$i=$page;
		if(!empty($list)){		
		...
		$list_view = $CI->parser->parse('admin_view/user/index',$data,true);
		return $list_view;
	}
	...
	public function new_user()
	{
		...
		$data = array(
				'title' => 'Add User',
				'roles' => $CI->user->get_role_list()
			);
		$productForm = $CI->parser->parse('admin_view/user/add',$data,true);
		return $productForm;
	}
	...
	public function get_edit_data($user_id)
	{
		$CI =& get_instance();
		$CI->load->model('admin/user');
		$edit_data = $CI->user->retrieve_edit_data($user_id);
		$data = array();
		if(!empty($edit_data)){
		$role = $CI->user->get_role_list();
		if(!empty($role)){
			foreach($role as $k=>$v){
		...
		$profile_data = $CI->parser->parse('admin_view/user/edit',$data,true);
		return $profile_data;
	}
	...
	public function edit_profile_form()
	{
		$CI =& get_instance();
		$CI->load->model('admin/user');
		$edit_data = $CI->user->profile_edit_data();	
		$data = array(
				'first_name' => $edit_data[0]['first_name'],
				'last_name' => $edit_data[0]['last_name'],
				'user_name' => $edit_data[0]['username']
			);	
		$profile_data = $CI->parser->parse('admin_view/user/edit_profile',$data,true);
		return $profile_data;
	}
}
?>